...
"""Segmentation dataset helpers and transformation utilities."""
...
class BrodenDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.resdir = os.path.join(
            self.directory, "broden%d_%d" % (broden_version, resolution)
        )
        ...
        with open(os.path.join(self.resdir, "category.csv"), encoding="utf-8") as f:
        ...
        self.image = [row for row in all_images if index_has_any_data(row, categories)]
        if size is not None:
            self.image = self.image[:size]
        with open(os.path.join(self.resdir, "label.csv"), encoding="utf-8") as f:
        ...
        for cat in self.categories:
            with open(
                os.path.join(self.resdir, "c_%s.csv" % cat), encoding="utf-8"
            ) as f:
            ...
            _, self.label_category[i] = max(
                (
                    (
                        ...
                        if i < len(self.category_map[cat]) and self.category_map[cat][i]
                        else 0
                    ),
                    ic,
                )
        ...
        extracted_textures = []
        extracted_scenes = []
        for label in self.label_info[1:]:
            for cat in categories:
                if label["category"].get(cat):
                    if cat == "material":
                        transformed_label = label["name"] + " material"
                        extracted_materials.append(transformed_label)
                    elif cat == "part" or cat == "object":
                        if label["name"] not in extracted_objects:
                            extracted_objects.append(label["name"])
                    elif cat == "color":
                        root = label["name"].replace("-c", "")
                        transformed_label = root + " color"
                        extracted_colors.append(transformed_label)
                    elif cat == "texture":
                        transformed_label = label["name"] + " texture"
                        extracted_textures.append(transformed_label)
                    elif cat == "scene":
                        root = label["name"].replace("-s", "")
                        root = root.replace("_", " ")
                        root = root.replace("-outdoor", " outdoor")
                        root = root.replace("-indoor", " indoor")
                        root = root.replace("-", " ")
                        transformed_label = root + " scene"
                        extracted_scenes.append(transformed_label)
    ...
    def __getitem__(self, idx):
        ...
        #    'scene': [], 'texture': []}
        image = self.loader(os.path.join(self.resdir, "images", record["image"]))
        segment = numpy.zeros(
            shape=(self.max_segment_depth, record["sh"], record["sw"]), dtype=int
        )
        ...
        for cat in self.categories:
            for layer in record[cat]:
                if isinstance(layer, int):
                    ...
                    png = numpy.asarray(
                        self.loader(os.path.join(self.resdir, "images", layer))
                    )
                    segment[depth, :, :] = png[:, :, 0] + png[:, :, 1] * 256
                    if self.include_bincount:
                        bincount += numpy.bincount(
                            segment[depth, :, :].flatten(), minlength=self.num_labels
                        )
                depth += 1
...
def create_subset_index(subset_name, index_file):
    ...
    with open(index_file) as infile, open(
        f"{dir_path}/index_{subset_name}.csv", "w"
    ) as outfile:
...
class ScaleSegmentation:
    ...
    def __call__(self, seg):
        ratio = (
            1,
            self.target_height / float(seg.shape[1]),
